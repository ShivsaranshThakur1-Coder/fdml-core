#!/usr/bin/env python3
"""M22 validator expansion with context/structure coherence checks and taxonomy burn-down support."""
...
COHERENCE_RULE_SPECS = [
    {
        "key": "rule:m22_note_source_id_matches_filename",
        "name": "m22_note_source_id_matches_filename",
        "description": "M22 uplift notes must carry source_id matching the FDML file source id.",
        "failureCode": "m22_note_source_id_mismatch",
    },
    {
        "key": "rule:m22_note_descriptor_pairs_present",
        "name": "m22_note_descriptor_pairs_present",
        "description": "M22 uplift notes must include at least one descriptor key/lexeme pair.",
        "failureCode": "m22_note_descriptor_pairs_missing",
    },
    {
        "key": "rule:m22_note_keys_within_low_support_set",
        "name": "m22_note_keys_within_low_support_set",
        "description": "M22 uplift note keys must be restricted to low-support descriptor families.",
        "failureCode": "m22_note_key_not_low_support",
    },
    {
        "key": "rule:m22_note_lexemes_grounded_in_source_text",
        "name": "m22_note_lexemes_grounded_in_source_text",
        "description": "M22 uplift lexemes recorded in notes must be present in acquired source text.",
        "failureCode": "m22_note_lexeme_not_in_source_text",
    },
]


NOTE_MARKER = "M22_UPLIFT"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Compose an M22 validator expansion report by augmenting M21 validator outputs "
            "with source-grounded context/structure alignment and M22 uplift-coherence rules."
        )
    )
    ap.add_argument(
        "--input-dir",
    ...
    ap.add_argument(
        ...
        help="M22 descriptor uplift report used to load low-support key set",
    )
    ap.add_argument(
        "--source-text-dir",
        action="append",
    ...
    ap.add_argument(
        ...
        help="output path for M22 validator expansion report",
    )
    ap.add_argument("--label", default="m22-validator-expansion-live", help="report label")
    ...
    ap.add_argument(
        ...
        type=int,
        default=200,
        help="minimum combined applicable-file count across all M22 rules",
    )
    return ap.parse_args()


def fail(msg: str) -> int:
    print(f"m22_validator_expansion.py: {msg}", file=sys.stderr)
    return 2
...
def parse_m22_note_entries(root: ET.Element) -> list[dict[str, Any]]:
...
def evaluate_coherence_rules(
    ...
    for file_path in files:
        ...
        if root.tag != "fdml":
            raise RuntimeError(f"root element is not <fdml> in {file_path}")
        file_key = file_path.resolve().as_posix()
        notes = parse_m22_note_entries(root)
    ...
    for spec in COHERENCE_RULE_SPECS:
        ...
        for file_path in files:
            ...
            elif rule_key == "rule:m22_note_keys_within_low_support_set":
                pass_ok = True
                for note in notes:
            ...
            elif rule_key == "rule:m22_note_lexemes_grounded_in_source_text":
                pass_ok = True
                for note in notes:
                    for pair in as_list(note.get("pairs")):
...
def main() -> int:
    ...
    checks = [
        ...
        {
            "id": "m22_coherence_rules_added",
            "ok": len(coherence_rules) >= len(COHERENCE_RULE_SPECS),
            "detail": f"m22_coherence_rules={len(coherence_rules)} expected={len(COHERENCE_RULE_SPECS)}",
        },
        {
    ...
    report = {
        ...
        "totals": {
            ...
            "m22SourceGroundedApplicable": total_applicable,
            "m22AlignmentRuleCount": len(alignment_rules),
            "m22CoherenceRuleCount": len(coherence_rules),
            "m22CoherenceApplicable": coherence_applicable,
        },
        "priorityCoverage": {
            ...
            "m22DescriptorTargetKeys": [
                str(as_list(spec.get("derivedFromCandidateKeys"))[0]) for spec in ALIGNMENT_RULE_SPECS
            ],
            "m22DescriptorRulePassRates": alignment_ratio_by_rule,
            "m22DescriptorApplicableByRule": alignment_applicable_by_rule,
            "m22CoherenceRulePassRates": coherence_ratio_by_rule,
            "m22CoherenceApplicableByRule": coherence_applicable_by_rule,
            "m22LowSupportKeys": low_support_keys,
        },
        "failureTaxonomy": [
    ...
    print(
        f"M22 VALIDATOR EXPANSION {'PASS' if ok else 'FAIL'} "
