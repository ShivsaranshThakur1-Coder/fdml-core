#!/usr/bin/env python3
"""Governance gate for M22 descriptor uplift and validator coherence adoption."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Validate M22 descriptor uplift plus validator coherence pipeline and emit "
            "decision/assumption/risk ledgers."
        )
    )
    ap.add_argument("--descriptor-report", required=True, help="path to M22 descriptor uplift report")
    ap.add_argument("--descriptor-registry-report", required=True, help="path to M22 descriptor registry report")
    ap.add_argument("--descriptor-coverage-report", required=True, help="path to M22 descriptor coverage report")
    ap.add_argument("--validator-baseline-report", required=True, help="path to M22 baseline validator report")
    ap.add_argument("--validator-current-report", required=True, help="path to M22 current validator report")
    ap.add_argument("--burndown-report", required=True, help="path to M22 validator burndown report")
    ...
    ap.add_argument("--label", default="m22-pipeline-governance", help="report label")

    ap.add_argument(
        "--required-source-dir",
        default="out/m21_descriptor_completion/run1",
        help="required canonical source corpus dir for M22 descriptor uplift and validator baseline",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help="required baseline coverage report path used by M22 descriptor uplift",
    )
    ap.add_argument(
        "--required-base-validator-report",
        default="out/m21_validator_expansion_report.json",
        help="required M21 validator report used as mapping baseline in M22 validator runs",
    )
    ap.add_argument(
        "--required-descriptor-report-path",
        default="out/m22_descriptor_uplift_report.json",
        help="required descriptor report path used by M22 validator runs",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default="out/m22_validator_expansion_baseline_report.json",
        help="required M22 baseline validator report path used by burndown",
    )
    ap.add_argument(
        "--required-current-report",
        default="out/m22_validator_expansion_report.json",
        help="required M22 current validator report path used by burndown",
    )
...
def fail(msg: str) -> int:
    print(f"m22_pipeline_governance_gate.py: {msg}", file=sys.stderr)
    return 2
...
def build_decision_registry(required_source_dir: str, required_descriptor_out_dir: str) -> list[dict[str, Any]]:
    return [
        {
            "id": "M22-DEC-001",
            "decision": "Keep one canonical baseline/current corpus path for M22 descriptor uplift and validator coherence",
            ...
            "alternativesConsidered": [
                ...
                "regionalized M22 sub-pipelines",
            ],
            "tradeoffs": [
                "pro: deterministic baseline/current comparability across all 109 files",
                "con: all M22 improvements must fit one shared FDML structure",
            ],
            "reversalCondition": "if canonical paths change, update all M22 stage inputs atomically",
        },
        {
            "id": "M22-DEC-002",
            "decision": "Preserve low-support uplift as an explicit stage artifact",
            "chosenOption": f"descriptor uplift output fixed at {required_descriptor_out_dir}",
            "alternativesConsidered": [
                "in-place edits on M21 corpus without a stage artifact",
                "validator-only changes without descriptor uplift",
            ],
            "tradeoffs": [
        ...
        },
        {
            ...
            "chosenOption": "compose M22 validator layer from 16 alignment rules plus 4 note-coherence rules",
            "alternativesConsidered": [
        ...
        {
            "id": "M22-DEC-004",
            "decision": "Promote M22 governance to CI",
            "chosenOption": "wire m22-pipeline-governance-check into ci target",
            "alternativesConsidered": [
        ...
        {
            "id": "M22-DEC-005",
            "decision": "Synchronize release evidence docs with M22 machine artifacts",
            "chosenOption": "update submission/program docs with explicit M22 commands and artifact paths",
            "alternativesConsidered": [
                "keep release docs at M21 snapshot",
                "store M22 evidence only in tracker files",
            ],
            "tradeoffs": [
...
def build_assumption_registry(
    ...
    return [
        {
            "id": "M22-ASM-001",
            "assumption": "M21 descriptor-completion corpus remains canonical baseline for M22",
            "confidence": 0.9,
            "verificationPlan": f"enforce descriptor and baseline validator inputs reference {required_source_dir}",
            "invalidationSignal": "any M22 stage reads from a different baseline corpus path",
        },
        {
            "id": "M22-ASM-002",
            "assumption": "M22 descriptor-uplift output remains canonical current corpus",
            "confidence": 0.9,
            "verificationPlan": f"enforce current coverage/validator inputs reference {required_descriptor_out_dir}",
            "invalidationSignal": "coverage or validator current inputs diverge from descriptor uplift output",
        },
        {
            "id": "M22-ASM-003",
            "assumption": "Low-support uplift thresholds reflect real descriptor support improvements",
            "confidence": 0.85,
        ...
        {
            "id": "M22-ASM-004",
            "assumption": "M22 burndown reflects genuine coherence-linked validator gains",
            "confidence": 0.85,
        ...
        {
            "id": "M22-ASM-005",
            "assumption": "Program and submission docs stay synchronized with M22 outputs",
            "confidence": 0.75,
            "verificationPlan": "require PRG-221/PRG-222/PRG-223 outcomes in PROGRAM_PLAN and M22 references in SUBMISSION",
            "invalidationSignal": "governance passes while release docs omit M22 artifacts",
        },
    ]
...
def build_risk_ledger(
    ...
    return [
        {
            "id": "M22-RSK-001",
            "risk": "Low-support descriptor families can remain uneven despite uplift gains",
            "severity": "high",
            "likelihood": "medium",
            "signal": (
            ...
            "mitigation": "continue evidence-driven uplift for residual low-support families and monitor per-key backlog",
            "owner": "descriptor",
        },
        {
            "id": "M22-RSK-002",
        ...
        {
            "id": "M22-RSK-004",
            "risk": "CI can drift from intended M22 governance without explicit wiring checks",
            "severity": "medium",
            "likelihood": "medium",
            "signal": "ci target missing m22-pipeline-governance-check",
            "mitigation": "gate on Makefile target existence and CI inclusion",
            "owner": "ci",
        },
        {
            "id": "M22-RSK-005",
            "risk": "Release evidence can become stale as M22 checks evolve",
            "severity": "medium",
            "likelihood": "medium",
            "signal": "submission doc lacks M22 command/evidence references",
...
def main() -> int:
    ...
    validator_total_applicable = as_int(validator_current_totals.get("m22SourceGroundedApplicable"), 0)
    validator_alignment_rule_count = as_int(validator_current_totals.get("m22AlignmentRuleCount"), 0)
    validator_coherence_rule_count = as_int(validator_current_totals.get("m22CoherenceRuleCount"), 0)
    validator_coherence_applicable = as_int(validator_current_totals.get("m22CoherenceApplicable"), 0)
    ...
    m22_descriptor_block = extract_make_target_block(make_text, "m22-descriptor-uplift-check")
    m22_validator_block = extract_make_target_block(make_text, "m22-validator-expansion-check")
    m22_governance_block = extract_make_target_block(make_text, "m22-pipeline-governance-check")
    ...
    add_check("m22_descriptor_report_ok", as_bool(descriptor.get("ok"), False), f"descriptor_ok={as_bool(descriptor.get('ok'), False)}")
    add_check(
        "m22_descriptor_counts_min",
    ...
    add_check(
        "m22_descriptor_source_grounded_additions_min",
    ...
    add_check(
        "m22_descriptor_low_support_thresholds",
    ...
    add_check(
        "m22_descriptor_low_support_ratio_increase",
    ...
    add_check(
        "m22_descriptor_stage_dir_invariants",
    ...
    add_check(
        ...
        "descriptor checks include low-support uplift, quality, and source-text coverage assertions",
    )
    add_check(
        "m22_descriptor_registry_counts_min",
    ...
    add_check("m22_descriptor_coverage_report_ok", as_bool(descriptor_coverage.get("ok"), False), f"coverage_ok={as_bool(descriptor_coverage.get('ok'), False)}")
    add_check(
        "m22_descriptor_coverage_depth_thresholds",
    ...
    add_check(
        "m22_descriptor_coverage_checks_present",
    ...
    add_check("m22_validator_baseline_ok", as_bool(validator_baseline.get("ok"), False), f"validator_baseline_ok={as_bool(validator_baseline.get('ok'), False)}")
    add_check("m22_validator_current_ok", as_bool(validator_current.get("ok"), False), f"validator_current_ok={as_bool(validator_current.get('ok'), False)}")
    add_check(
        "m22_validator_counts_min",
    ...
    add_check(
        "m22_validator_candidate_mapping_complete",
    ...
    add_check(
        "m22_validator_applicability_ok",
    ...
    add_check(
        "m22_validator_coherence_rules_added",
        check_ok(validator_baseline, "m22_coherence_rules_added")
        and check_ok(validator_current, "m22_coherence_rules_added"),
        "m22 coherence rule-addition check passes for baseline/current",
    )
    add_check(
        "m22_validator_input_invariants",
    ...
    add_check("m22_burndown_ok", as_bool(burndown.get("ok"), False), f"burndown_ok={as_bool(burndown.get('ok'), False)}")
    add_check(
        "m22_burndown_reduction_ratio_min",
        reduction_ratio >= args.min_reduction_ratio,
        f"reduction_ratio={reduction_ratio:.4f} min={args.min_reduction_ratio:.4f}",
    )
    add_check(
        "m22_burndown_failure_file_ratio_max",
    ...
    add_check(
        "m22_make_target_invariants",
        required_source_dir in m22_descriptor_block
        and required_descriptor_out_dir in m22_descriptor_block
        and required_baseline_coverage_report in m22_descriptor_block
        and required_source_dir in m22_validator_block
        and required_descriptor_out_dir in m22_validator_block
        and required_base_validator_report in m22_validator_block
        and required_descriptor_report_path in m22_validator_block
        and required_baseline_report in m22_validator_block
        and required_current_report in m22_validator_block,
        "M22 make targets reference required dirs, reports, and baseline/current stage paths",
    )
    add_check(
        "make_ci_wires_m22_governance",
        "m22-pipeline-governance-check" in ci_line,
        "ci target includes m22-pipeline-governance-check",
    )
    add_check(
        "make_ci_keeps_m21_governance",
        "m21-pipeline-governance-check" in ci_line,
        "ci target retains m21-pipeline-governance-check",
    )
    add_check(
        "make_m22_targets_exist",
        bool(m22_descriptor_block.strip()) and bool(m22_validator_block.strip()) and bool(m22_governance_block.strip()),
        "m22 descriptor/validator/governance target blocks exist",
    )
    add_check(
        "program_plan_mentions_prg221_outcome",
        "M22 execution outcome (PRG-221)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-221 execution outcome",
    )
    add_check(
        "program_plan_mentions_prg222_outcome",
        "M22 execution outcome (PRG-222)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-222 execution outcome",
    )
    add_check(
        "program_plan_mentions_prg223_outcome",
        "M22 execution outcome (PRG-223)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-223 execution outcome",
    )
    add_check(
        "submission_doc_mentions_m22_release",
        "m22-pipeline-governance-check" in submission_text
        and "out/m22_pipeline_governance.json" in submission_text,
        "SUBMISSION.md includes M22 release command and governance artifact",
    )
    add_check(
