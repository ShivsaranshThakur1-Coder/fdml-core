#!/usr/bin/env python3
"""Deterministic M29 release-workflow baseline and prioritized execution backlog."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Generate deterministic M29 release-workflow baseline and execution backlog."
    )
    ...
    ap.add_argument("--report-out", default="out/m29_release_baseline_report.json")
    ap.add_argument("--required-active-milestone", default="M29")
    ap.add_argument("--required-previous-milestone", default="M28")
    ap.add_argument("--required-work-id", default="PRG-276")
    ap.add_argument("--required-next-work-id", default="PRG-277")
    ap.add_argument("--required-governance-work-id", default="PRG-278")
    ap.add_argument("--required-ci-target", default="m29-release-baseline-check")
...
def main() -> int:
    ...
    checks.append(
        {
            "id": "step_map_contains_prg276_command",
            "ok": mapped_ok,
    ...
    program_plan_check = doc_token_check(
        ...
        ["M29", "PRG-276", args.required_ci_target, "out/m29_release_baseline_report.json"],
    )
    usage_check = doc_token_check(
        Path(args.usage_doc),
        ["PRG-276", f"make {args.required_ci_target}", "out/m29_release_baseline_report.json"],
    )
    ...
    open_m29_rows = [row for row in m29_rows if row.get("status", "") in OPEN_STATUSES]
        ...
        backlog.append(
            {
                ...
                "acceptanceTest": (
                    ...
                    "and reduce avoidable working-tree noise before freeze."
                ),
            }
        )
        ...
        backlog.append(
            {
                ...
                "linkedWorkItemId": "PRG-277",
                "title": "Synchronize M29 release-flow command and artifact references",
                "evidence": (
                    "docsWithGaps="
                    + ",".join(check["path"] for check in docs_with_gaps)
                ),
                "acceptanceTest": (
                    "PROGRAM_PLAN, SUBMISSION, and USAGE reference the same M29 "
        ...
        backlog.append(
            {
                ...
                "linkedWorkItemId": "PRG-277",
                "title": "Execute release-workflow stabilization backlog and rerun rehearsal",
                "evidence": f"releaseReady={release_ready} queuedGapCount={queued_gap_count}",
                "acceptanceTest": (
                    "Apply prioritized M29 fixes, rerun deterministic release workflow checks, "
        ...
        backlog.append(
            {
                "id": "M29-BLG-004",
                "priority": 2,
                "area": "governance_freeze",
                "linkedWorkItemId": "PRG-278",
                "title": "Adopt M29 anti-drift governance and freeze final delivery package",
                "evidence": f"{args.required_governance_work_id}.status={status_governance!r}",
                "acceptanceTest": (
                    "Add M29 governance gate to CI and lock final delivery package "
                    "reproducibility with deterministic artifacts."
                ),
            }
        )

    if len(open_m29_rows) > 0:
        backlog.append(
            {
                "id": "M29-BLG-005",
                "priority": 3,
                "area": "queue_burndown",
                "linkedWorkItemId": "PRG-277",
                "title": "Burn down remaining open M29 queue items after baseline",
                "evidence": f"openM29Rows={len(open_m29_rows)}",
                "acceptanceTest": (
                    "Drive open M29 rows to zero with passing CI and synchronized handoff docs."
                ),
            }
        )
    ...
    report = {
        "schemaVersion": "1",
        "label": "m29-release-workflow-baseline-live",
        "inputs": {
            "plan": args.plan,
            "work": args.work,
        ...
        "metrics": {
            ...
            "openM29QueueCount": len(open_m29_rows),
