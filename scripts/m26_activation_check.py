#!/usr/bin/env python3
"""M26 activation gate for post-closeout production polish queue bootstrap."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Validate M26 activation and seeded work queue state.")
    ...
    ap.add_argument("--report-out", default="out/m26_activation_report.json")
    ap.add_argument("--required-active-milestone", default="M26")
    ap.add_argument("--required-previous-milestone", default="M25")
    ap.add_argument("--required-activation-work-id", default="PRG-260")
    ap.add_argument("--required-next-work-id", default="PRG-261")
...
def main() -> int:
    ...
    m26_rows = [r for r in rows if r.get("milestone_id", "") == args.required_active_milestone]
    m26_open = [r for r in m26_rows if r.get("status", "") in {"planned", "in_progress", "blocked"}]
    checks.append(
        {
            "id": "m26_active_queue_seeded",
            "ok": len(m26_open) >= args.min_active_queue,
            "detail": f"open_m26_rows={len(m26_open)} min={args.min_active_queue}",
        }
    )
    ...
    checks.append(
        {
            ...
            "ok": "m26-activation-check:" in makefile_text,
            "detail": "Makefile contains m26-activation-check target",
        }
    )
    checks.append(
        {
            "id": "make_ci_wires_m26_activation",
            "ok": "ci:" in makefile_text and "m26-activation-check" in makefile_text,
            "detail": "Makefile ci target includes m26-activation-check",
        }
    )

    checks.append(
        {
            "id": "program_plan_mentions_m26",
            "ok": has_all(
                program_plan_text,
                [
                    "M26",
            ...
            "detail": "PROGRAM_PLAN.md includes M26 activation state and next step",
        }
    )
