#!/usr/bin/env python3
"""Deterministic M23 source-grounded descriptor-support consolidation."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Apply deterministic source-grounded descriptor consolidation for residual "
            "low-support and high-gap descriptor families with strict quality checks."
        )
    )
    ap.add_argument("--source-dir", default="out/m22_descriptor_uplift/run1", help="input FDML directory")
    ap.add_argument(
    ...
    ap.add_argument("--out-dir", default="out/m23_descriptor_consolidation/run1", help="output FDML directory")
    ap.add_argument(
        "--report-out",
    ...
    ap.add_argument("--label", default="m23-descriptor-consolidation-live", help="report label")
    ap.add_argument("--min-total-files", type=int, default=100)
    ap.add_argument("--min-files-updated", type=int, default=30)
    ap.add_argument("--min-source-grounded-additions", type=int, default=60)
    ap.add_argument("--max-low-support-ratio", type=float, default=0.75)
    ap.add_argument("--min-low-support-keys", type=int, default=6)
    ap.add_argument("--min-low-support-keys-with-growth", type=int, default=6)
    ap.add_argument(
        "--min-potential-growth-gap",
        type=int,
        default=8,
        help="minimum source-signal minus support gap to retain high-gap keys even above ratio threshold",
    )
...
def fail(msg: str) -> int:
    print(f"m23_descriptor_consolidation.py: {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    if args.min_potential_growth_gap < 0:
        return fail("--min-potential-growth-gap must be >= 0")
    ...
    for key in ALL_KEYS:
        ...
        if potential_growth > 0 and (
            ratio <= args.max_low_support_ratio or potential_growth >= args.min_potential_growth_gap
        ):
    ...
    low_support_candidates.sort(
        key=lambda row: (-int(row["potentialGrowth"]), float(row["beforeRatio"]), str(row["key"]))
    )
    low_support_keys = [str(row["key"]) for row in low_support_candidates]
    high_gap_keys = [
        str(row["key"]) for row in low_support_candidates if int(row["potentialGrowth"]) >= args.min_potential_growth_gap
    ]
    ...
    for file_path in files:
        ...
        if selected_additions:
            ...
            node.text = (
                f"M23_UPLIFT source_id={source_id} low_support_source_grounded_descriptors: "
    ...
    checks = [
        ...
        {
            "id": "high_gap_keys_present",
            "ok": len(high_gap_keys) > 0,
            "detail": f"high_gap_keys={len(high_gap_keys)} min_potential_growth_gap={args.min_potential_growth_gap}",
        },
        {
    ...
    report = {
        ...
        "thresholds": {
            ...
            "minPotentialGrowthGap": args.min_potential_growth_gap,
