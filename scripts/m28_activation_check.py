#!/usr/bin/env python3
"""M28 activation gate for post-M27 transition and queue bootstrap."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Validate M28 activation and seeded work queue state.")
    ...
    ap.add_argument("--report-out", default="out/m28_activation_report.json")
    ap.add_argument("--required-active-milestone", default="M28")
    ap.add_argument("--required-previous-milestone", default="M27")
    ap.add_argument("--required-activation-work-id", default="PRG-270")
    ap.add_argument("--required-next-work-id", default="PRG-271")
...
def main() -> int:
    ...
    m28_open = [r for r in m28_rows if r.get("status", "") in {"planned", "in_progress", "blocked"}]
    checks.append(
        {
            "id": "m28_active_queue_seeded",
            "ok": len(m28_open) >= args.min_active_queue,
            "detail": f"open_m28_rows={len(m28_open)} min={args.min_active_queue}",
        }
    )
    ...
    checks.append(
        {
            ...
            "ok": "m28-activation-check:" in makefile_text,
            "detail": "Makefile contains m28-activation-check target",
        }
    )
    checks.append(
        {
            "id": "make_ci_wires_m28_activation",
            "ok": "ci:" in makefile_text and "m28-activation-check" in makefile_text,
            "detail": "Makefile ci target includes m28-activation-check",
        }
    )

    checks.append(
        {
            "id": "program_plan_mentions_m28",
            "ok": has_all(
                program_plan_text,
                [
                    "M28",
            ...
            "detail": "PROGRAM_PLAN.md includes M28 activation state and next step",
        }
    )
