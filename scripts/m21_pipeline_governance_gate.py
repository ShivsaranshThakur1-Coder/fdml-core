#!/usr/bin/env python3
"""Governance gate for M21 descriptor-completion and context/structure validator adoption."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Validate M21 descriptor-completion plus context/structure validator pipeline "
            "and emit decision/assumption/risk ledgers."
        )
    )
    ap.add_argument("--descriptor-report", required=True, help="path to M21 descriptor-completion report")
    ap.add_argument("--descriptor-registry-report", required=True, help="path to M21 descriptor registry report")
    ap.add_argument("--descriptor-coverage-report", required=True, help="path to M21 descriptor coverage report")
    ap.add_argument("--validator-m17-baseline-report", required=True, help="path to M21 baseline M17 validator report")
    ap.add_argument("--validator-m17-current-report", required=True, help="path to M21 current M17 validator report")
    ap.add_argument("--validator-baseline-report", required=True, help="path to M21 baseline validator report")
    ap.add_argument("--validator-current-report", required=True, help="path to M21 current validator report")
    ap.add_argument("--burndown-report", required=True, help="path to M21 validator burndown report")
    ...
    ap.add_argument("--label", default="m21-pipeline-governance", help="report label")

    ap.add_argument(
        "--required-source-dir",
        default="out/m20_descriptor_evidence/run1",
        help="required canonical source corpus dir for M21 descriptor completion",
    )
    ap.add_argument(
        "--required-descriptor-out-dir",
        default="out/m21_descriptor_completion/run1",
        help="required descriptor-completion output dir",
    )
    ap.add_argument(
        "--required-baseline-coverage-report",
        default="out/m20_fdml_coverage_report.json",
        help="required M20 baseline coverage report path used by M21 descriptor completion",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default="out/m21_validator_expansion_baseline_m17_report.json",
        help="required M17 baseline report path used by M21 baseline validator layer",
    )
    ap.add_argument(
        "--required-m17-current-report",
        default="out/m21_validator_expansion_m17_report.json",
        help="required M17 current report path used by M21 current validator layer",
    )
    ap.add_argument(
        "--required-baseline-report",
        default="out/m21_validator_expansion_baseline_report.json",
        help="required M21 baseline validator report for burndown",
    )
    ap.add_argument(
        "--required-current-report",
        default="out/m21_validator_expansion_report.json",
        help="required M21 current validator report for burndown",
    )

    ap.add_argument("--min-total-files", type=int, default=100, help="minimum source files")
    ap.add_argument("--min-descriptor-files-updated", type=int, default=12)
    ap.add_argument("--min-source-grounded-additions", type=int, default=18)
    ap.add_argument("--min-descriptor-keys-with-growth", type=int, default=8)
    ap.add_argument("--min-style-depth-gain", type=int, default=4)
    ap.add_argument("--min-cultural-depth-gain", type=int, default=4)
    ap.add_argument("--min-combined-depth-gain", type=int, default=3)
    ...
    ap.add_argument("--min-files-with-cultural-depth", type=int, default=106)
    ap.add_argument("--min-files-with-combined-depth", type=int, default=103)
    ap.add_argument("--min-expanded-rules", type=int, default=12)
    ap.add_argument("--min-source-grounded-applicable", type=int, default=200)
    ap.add_argument("--min-reduction-ratio", type=float, default=0.05)
    ap.add_argument("--max-failure-file-ratio", type=float, default=0.90)
...
def fail(msg: str) -> int:
    print(f"m21_pipeline_governance_gate.py: {msg}", file=sys.stderr)
    return 2
...
def build_decision_registry(required_source_dir: str, required_descriptor_out_dir: str) -> list[dict[str, Any]]:
    return [
        {
            "id": "M21-DEC-001",
            "decision": "Keep one canonical baseline/current corpus path for M21 completion and validator burn-down",
            ...
            "alternativesConsidered": [
                ...
                "run per-region variants for M21 governance",
            ],
            "tradeoffs": [
                "pro: deterministic baseline/current comparability across all 109 files",
                "con: all descriptor and validator gains must fit one shared FDML structure",
            ],
            "reversalCondition": "if canonical paths change, update all M21 stage inputs atomically",
        },
        {
            "id": "M21-DEC-002",
            "decision": "Preserve descriptor-completion as an explicit stage artifact",
            "chosenOption": f"descriptor completion output fixed at {required_descriptor_out_dir}",
            "alternativesConsidered": [
                "in-place edits on M20 corpus without stage artifact",
                "validator-only improvements without descriptor depth pass",
            ],
            "tradeoffs": [
                "pro: auditable evidence of depth gains before validator evaluation",
        ...
        {
            "id": "M21-DEC-003",
            "decision": "Evaluate context/structure constraints as a dedicated M21 validator layer",
            "chosenOption": "compose M21 rules from M17 baseline/current reports plus 16 source-grounded context/structure checks",
            "alternativesConsidered": [
                "merge M21 constraints into legacy totals without dedicated layer",
                "evaluate context/structure outside validator framework",
            ],
            "tradeoffs": [
                "pro: isolates measurable M21 constraint progress with explicit applicability",
                "con: governance must enforce multi-report composition invariants",
            ],
            "reversalCondition": "if layered composition becomes unstable, migrate to single-pass unified validator output",
        },
        {
            "id": "M21-DEC-004",
            "decision": "Promote M21 governance to CI",
            "chosenOption": "wire m21-pipeline-governance-check into ci target",
            "alternativesConsidered": [
        ...
        {
            "id": "M21-DEC-005",
            "decision": "Synchronize release evidence docs with M21 machine artifacts",
            "chosenOption": "update submission/program docs with explicit M21 commands and artifact paths",
            "alternativesConsidered": [
                "keep release docs at M20 snapshot",
                "store M21 evidence only in tracker files",
            ],
            "tradeoffs": [
                "pro: evaluator-facing release story stays current and reproducible",
                "con: docs require ongoing synchronization",
            ],
            "reversalCondition": "if release handoff format changes, migrate references to the canonical release doc",
        },
    ]
...
def build_assumption_registry(
    ...
    return [
        {
            "id": "M21-ASM-001",
            "assumption": "M20 descriptor-evidence corpus remains canonical baseline for M21",
            "confidence": 0.9,
            "verificationPlan": f"enforce descriptor and baseline validator inputs reference {required_source_dir}",
            "invalidationSignal": "any M21 stage reads from a different baseline corpus path",
        },
        {
            "id": "M21-ASM-002",
            "assumption": "M21 descriptor-completion output remains canonical current corpus",
            "confidence": 0.9,
            "verificationPlan": f"enforce current coverage/validator inputs reference {required_descriptor_out_dir}",
            "invalidationSignal": "coverage or validator current inputs diverge from descriptor completion output",
        },
        {
            "id": "M21-ASM-003",
            "assumption": "M21 depth gain thresholds reflect real descriptor completion progress",
            "confidence": 0.85,
            "verificationPlan": "require files-updated, source-grounded additions, and style/culture/combined depth gains",
            "invalidationSignal": "thresholds pass while descriptor support regresses or quality fails",
        },
        {
            "id": "M21-ASM-004",
            "assumption": "M21 baseline/current burndown reflects real context/structure validator gains",
            "confidence": 0.85,
        ...
        {
            "id": "M21-ASM-005",
            "assumption": "Program and submission docs stay synchronized with M21 outputs",
            "confidence": 0.75,
            "verificationPlan": "require PRG-211/PRG-212/PRG-213 outcomes in PROGRAM_PLAN and M21 references in SUBMISSION",
            "invalidationSignal": "governance passes while release docs omit M21 artifacts",
        },
    ]
...
def build_risk_ledger(
    ...
    style_depth_gain: int,
    cultural_depth_gain: int,
    combined_depth_gain: int,
    ...
    return [
        {
            "id": "M21-RSK-001",
            "risk": "Descriptor completion can still under-cover nuanced style/context semantics",
            "severity": "high",
            "likelihood": "medium",
            "signal": (
                f"filesUpdated={descriptor_files_updated} additions={source_grounded_additions} "
                f"styleGain={style_depth_gain} cultureGain={cultural_depth_gain} combinedGain={combined_depth_gain}"
            ),
            "mitigation": "continue source-grounded lexeme-family expansion and monitor per-key backlog",
            "owner": "descriptor",
        },
        {
            "id": "M21-RSK-002",
            "risk": "Context/structure rule layer may lose applicability on future corpus shifts",
            "severity": "medium",
            "likelihood": "medium",
            "signal": f"validatorRuleCount={validator_rule_count} sourceGroundedApplicable={validator_total_applicable}",
            "mitigation": "fail governance on applicability regression and keep baseline/current invariant checks",
            "owner": "validator",
        },
        {
            "id": "M21-RSK-003",
            "risk": "Burndown metrics can mask quality issues if mapping degrades",
            "severity": "high",
            "likelihood": "low",
            "signal": (
        ...
        {
            "id": "M21-RSK-004",
            "risk": "CI can drift from intended M21 governance without explicit wiring checks",
            "severity": "medium",
            "likelihood": "medium",
            "signal": "ci target missing m21-pipeline-governance-check",
            "mitigation": "gate on Makefile target existence and CI inclusion",
            "owner": "ci",
        },
        {
            "id": "M21-RSK-005",
            "risk": "Release evidence can become stale as M21 checks evolve",
            "severity": "medium",
            "likelihood": "medium",
            "signal": "submission doc lacks M21 command/evidence references",
...
def main() -> int:
    ...
    if args.min_style_depth_gain < 0 or args.min_cultural_depth_gain < 0 or args.min_combined_depth_gain < 0:
        return fail("--min-style-depth-gain, --min-cultural-depth-gain, and --min-combined-depth-gain must be >= 0")
    ...
    style_before = as_int(descriptor_totals.get("filesWithStyleDepthBefore"), 0)
    style_after = as_int(descriptor_totals.get("filesWithStyleDepthAfter"), 0)
    cultural_before = as_int(descriptor_totals.get("filesWithCulturalDepthBefore"), 0)
    cultural_after = as_int(descriptor_totals.get("filesWithCulturalDepthAfter"), 0)
    combined_before = as_int(descriptor_totals.get("filesWithCombinedDepthBefore"), 0)
    combined_after = as_int(descriptor_totals.get("filesWithCombinedDepthAfter"), 0)
    style_gain = style_after - style_before
    cultural_gain = cultural_after - cultural_before
    combined_gain = combined_after - combined_before
    ...
    validator_total_applicable = as_int(validator_current_totals.get("m21SourceGroundedApplicable"), 0)
    ...
    m21_descriptor_block = extract_make_target_block(make_text, "m21-descriptor-completion-check")
    m21_validator_block = extract_make_target_block(make_text, "m21-validator-expansion-check")
    m21_governance_block = extract_make_target_block(make_text, "m21-pipeline-governance-check")
    ...
    add_check("m21_descriptor_report_ok", as_bool(descriptor.get("ok"), False), f"descriptor_ok={as_bool(descriptor.get('ok'), False)}")
    add_check(
        "m21_descriptor_counts_min",
    ...
    add_check(
        "m21_descriptor_source_grounded_additions_min",
    ...
    add_check(
        "m21_descriptor_stage_dir_invariants",
    ...
    add_check(
        "m21_descriptor_depth_gain_min",
        style_gain >= args.min_style_depth_gain
        and cultural_gain >= args.min_cultural_depth_gain
        and combined_gain >= args.min_combined_depth_gain,
        (
            f"style_gain={style_gain} min_style={args.min_style_depth_gain} "
            f"cultural_gain={cultural_gain} min_cultural={args.min_cultural_depth_gain} "
            f"combined_gain={combined_gain} min_combined={args.min_combined_depth_gain}"
        ),
    )
    add_check(
        "m21_descriptor_checks_present",
        check_ok(descriptor, "files_updated_min")
        and check_ok(descriptor, "source_grounded_additions_min")
        and check_ok(descriptor, "descriptor_keys_with_growth_min")
        and check_ok(descriptor, "style_depth_gain_min")
        and check_ok(descriptor, "cultural_depth_gain_min")
        and check_ok(descriptor, "combined_depth_gain_min")
        and check_ok(descriptor, "doctor_pass_rate_min")
        and check_ok(descriptor, "geo_pass_rate_min"),
        "descriptor checks include update/addition/growth/depth/quality assertions",
    )

    add_check(
        "m21_descriptor_registry_counts_min",
    ...
    add_check("m21_descriptor_coverage_report_ok", as_bool(descriptor_coverage.get("ok"), False), f"coverage_ok={as_bool(descriptor_coverage.get('ok'), False)}")
    add_check(
        "m21_descriptor_coverage_depth_thresholds",
    ...
    add_check(
        "m21_descriptor_coverage_checks_present",
    ...
    add_check("m21_validator_m17_baseline_ok", as_bool(validator_m17_baseline.get("ok"), False), f"validator_m17_baseline_ok={as_bool(validator_m17_baseline.get('ok'), False)}")
    add_check("m21_validator_m17_current_ok", as_bool(validator_m17_current.get("ok"), False), f"validator_m17_current_ok={as_bool(validator_m17_current.get('ok'), False)}")
    add_check(
        "m21_validator_m17_input_invariants",
        ...
        (
            ...
            f"candidate_report_baseline={m17_baseline_candidate_report} "
            f"candidate_report_current={m17_current_candidate_report}"
        ),
    )
    add_check(
        "m21_validator_m17_mapping_complete",
    ...
    add_check("m21_validator_baseline_ok", as_bool(validator_baseline.get("ok"), False), f"validator_baseline_ok={as_bool(validator_baseline.get('ok'), False)}")
    add_check("m21_validator_current_ok", as_bool(validator_current.get("ok"), False), f"validator_current_ok={as_bool(validator_current.get('ok'), False)}")
    add_check(
        "m21_validator_counts_min",
    ...
    add_check(
        "m21_validator_candidate_mapping_complete",
    ...
    add_check(
        "m21_validator_applicability_ok",
    ...
    add_check(
        "m21_validator_context_structure_rules_added",
        check_ok(validator_baseline, "m21_context_structure_rules_added")
        and check_ok(validator_current, "m21_context_structure_rules_added"),
        "m21 context/structure rule-addition check passes for baseline/current",
    )
    add_check(
        "m21_validator_input_invariants",
    ...
    add_check("m21_burndown_ok", as_bool(burndown.get("ok"), False), f"burndown_ok={as_bool(burndown.get('ok'), False)}")
    add_check(
        "m21_burndown_reduction_ratio_min",
        reduction_ratio >= args.min_reduction_ratio,
        f"reduction_ratio={reduction_ratio:.4f} min={args.min_reduction_ratio:.4f}",
    )
    add_check(
        "m21_burndown_failure_file_ratio_max",
    ...
    add_check(
        "m21_make_target_invariants",
        required_source_dir in m21_descriptor_block
        and required_descriptor_out_dir in m21_descriptor_block
        and required_baseline_coverage_report in m21_descriptor_block
        and required_source_dir in m21_validator_block
        and required_descriptor_out_dir in m21_validator_block
        and required_candidate_report in m21_validator_block
        and required_m17_baseline_report in m21_validator_block
        and required_m17_current_report in m21_validator_block
        and required_baseline_report in m21_validator_block
        and required_current_report in m21_validator_block,
        "M21 make targets reference required dirs, reports, and baseline/current stage paths",
    )
    add_check(
        "make_ci_wires_m21_governance",
        "m21-pipeline-governance-check" in ci_line,
        "ci target includes m21-pipeline-governance-check",
    )
    add_check(
        "make_ci_keeps_m20_governance",
        "m20-pipeline-governance-check" in ci_line,
        "ci target retains m20-pipeline-governance-check",
    )
    add_check(
        "make_m21_targets_exist",
        bool(m21_descriptor_block.strip()) and bool(m21_validator_block.strip()) and bool(m21_governance_block.strip()),
        "m21 descriptor/validator/governance target blocks exist",
    )
    add_check(
        "program_plan_mentions_prg211_outcome",
        "M21 execution outcome (PRG-211)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-211 execution outcome",
    )
    add_check(
        "program_plan_mentions_prg212_outcome",
        "M21 execution outcome (PRG-212)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-212 execution outcome",
    )
    add_check(
        "program_plan_mentions_prg213_outcome",
        "M21 execution outcome (PRG-213)" in plan_text,
        "PROGRAM_PLAN.md includes PRG-213 execution outcome",
    )
    add_check(
        "submission_doc_mentions_m21_release",
        "m21-pipeline-governance-check" in submission_text
        and "out/m21_pipeline_governance.json" in submission_text,
        "SUBMISSION.md includes M21 release command and governance artifact",
    )
    add_check(
    ...
    payload: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "minStyleDepthGain": args.min_style_depth_gain,
            "minCulturalDepthGain": args.min_cultural_depth_gain,
            "minCombinedDepthGain": args.min_combined_depth_gain,
        ...
        "metrics": {
            ...
            "styleDepthBefore": style_before,
            "styleDepthAfter": style_after,
            "styleDepthGain": style_gain,
            "culturalDepthBefore": cultural_before,
            "culturalDepthAfter": cultural_after,
            "culturalDepthGain": cultural_gain,
            "combinedDepthBefore": combined_before,
            "combinedDepthAfter": combined_after,
            "combinedDepthGain": combined_gain,
