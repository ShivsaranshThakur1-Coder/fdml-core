#!/usr/bin/env python3
"""M21 validator expansion using source-grounded context and structure constraints."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Compose an M21 validator expansion report by augmenting M17 validator "
            "outputs with source-grounded context and structure constraints."
        )
    )
    ap.add_argument(
        "--input-dir",
    ...
    ap.add_argument(
        ...
        help="output path for M21 validator expansion report",
    )
    ap.add_argument("--label", default="m21-validator-expansion-live", help="report label")
    ap.add_argument("--min-total-files", type=int, default=100, help="minimum expected total files")
    ap.add_argument("--min-rules", type=int, default=12, help="minimum expanded rule count required")
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        type=int,
        default=120,
        help="minimum combined applicable-file count across all M21 rules",
    )
    return ap.parse_args()


def fail(msg: str) -> int:
    print(f"m21_validator_expansion.py: {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    # M21 focuses on source-grounded context and structure rule layer only.
    # Base report is used for candidate mapping integrity and context, not to carry
    # forward unrelated historical failures into M21 burndown math.
    ...
    checks = [
        ...
        {
            "id": "m21_context_structure_rules_added",
            "ok": len(new_rules) >= len(NEW_RULE_SPECS),
            "detail": f"m21_rules={len(new_rules)} expected={len(NEW_RULE_SPECS)}",
        },
        {
    ...
    report = {
        ...
        "totals": {
            ...
            "m21SourceGroundedApplicable": total_applicable,
        },
        "priorityCoverage": {
            ...
            "m21DescriptorTargetKeys": [
                str(as_list(spec.get("derivedFromCandidateKeys"))[0]) for spec in NEW_RULE_SPECS
            ],
            "m21DescriptorRulePassRates": m21_ratio_by_rule,
            "m21DescriptorApplicableByRule": m21_applicable_by_rule,
        },
        "failureTaxonomy": [
    ...
    print(
        f"M21 VALIDATOR EXPANSION {'PASS' if ok else 'FAIL'} "
