#!/usr/bin/env python3
"""M29 activation gate for post-M28 transition and queue bootstrap."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Validate M29 activation and seeded work queue state.")
    ...
    ap.add_argument("--report-out", default="out/m29_activation_report.json")
    ap.add_argument("--required-active-milestone", default="M29")
    ap.add_argument("--required-previous-milestone", default="M28")
    ap.add_argument("--required-activation-work-id", default="PRG-275")
    ap.add_argument("--required-next-work-id", default="PRG-276")
...
def main() -> int:
    ...
    m29_rows = [r for r in rows if r.get("milestone_id", "") == args.required_active_milestone]
    m29_open = [r for r in m29_rows if r.get("status", "") in {"planned", "in_progress", "blocked"}]
    checks.append(
        {
            "id": "m29_active_queue_seeded",
            "ok": len(m29_open) >= args.min_active_queue,
            "detail": f"open_m29_rows={len(m29_open)} min={args.min_active_queue}",
        }
    )
    ...
    checks.append(
        {
            ...
            "ok": "m29-activation-check:" in makefile_text,
            "detail": "Makefile contains m29-activation-check target",
        }
    )
    checks.append(
        {
            "id": "make_ci_wires_m29_activation",
            "ok": "ci:" in makefile_text and "m29-activation-check" in makefile_text,
            "detail": "Makefile ci target includes m29-activation-check",
        }
    )

    checks.append(
        {
            "id": "program_plan_mentions_m29",
            "ok": has_all(
                program_plan_text,
                [
                    "M29",
            ...
            "detail": "PROGRAM_PLAN.md includes M29 activation state and next step",
        }
    )
