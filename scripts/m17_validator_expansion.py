#!/usr/bin/env python3
"""Expanded full-corpus validator stack derived from M15 discovery candidates (M17 deepening)."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Run expanded M17 validator stack from discovery candidates and publish failure taxonomy."
    )
    ap.add_argument(
        "--input-dir",
    ...
    ap.add_argument(
        ...
        help="path to M15 validator candidate report (M17 reuses this evidence ledger)",
    )
    ap.add_argument(
        "--report-out",
    ...
    ap.add_argument(
        "--label",
        default="m17-validator-expansion",
        help="report label",
    )
    ap.add_argument(
        "--min-total-files",
...
def fail(msg: str) -> int:
    print(f"m17_validator_expansion.py: {msg}", file=sys.stderr)
    return 2
...
def collect_file_meta(files: list[Path]) -> dict[str, dict[str, Any]]:
    ...
    for file_path in files:
        ...
        steps_missing_who = 0
        steps_missing_action = 0
        steps_missing_primitive = 0
        steps_missing_start_foot = 0
        steps_missing_end_foot = 0
        step_foot_pairs_seen = 0
        steps_same_start_end_foot = 0
        step_count_sequence_violations = 0
        figures_without_steps = 0
        transition_total = 0
        transitions_with_foot_markers = 0
        transition_foot_continuity_violations = 0
        transitions_with_start_foot = 0
        transition_start_foot_repeat_violations = 0
        steps_non_positive_beats = 0
        step_beats_seen = 0
        jump_cue_step_total = 0
        jump_cue_step_positive_beats = 0
        turn_cue_step_total = 0
        turn_cue_step_axis = 0
        ...
        for figure in root.findall(".//figure"):
            ...
            for step in figure.findall("./step"):
                ...
                if not action:
                    steps_missing_action += 1
                if not attr(step, "who"):
                    steps_missing_who += 1
                ...
                if not attr(step, "count"):
                    ...
                    try:
                        count_value = int(attr(step, "count"))
                        if count_value != expected_count:
                            step_count_sequence_violations += 1
                    except Exception:
                        step_count_sequence_violations += 1
                ...
                start_foot_token = normalize_value(start_foot)
                end_foot_token = normalize_value(end_foot)
                start_foot_present = bool(start_foot) and not is_placeholder_value(start_foot)
                end_foot_present = bool(end_foot) and not is_placeholder_value(end_foot)
                if not start_foot_present:
                    steps_missing_start_foot += 1
                if not end_foot_present:
                    steps_missing_end_foot += 1
                if start_foot_present and end_foot_present:
                    step_foot_pairs_seen += 1
                    if start_foot_token == end_foot_token:
                        steps_same_start_end_foot += 1

                if step.find("./geo/primitive") is None:
                    steps_missing_primitive += 1

                has_axis_this_step = False
                has_turn_kind_primitive = False
                for prim in step.findall("./geo/primitive"):
                    if attr(prim, "axis"):
                        has_axis_this_step = True
                    if attr(prim, "kind") in {"turn", "twirl"}:
                        has_turn_kind_primitive = True
                ...
                has_jump_cue = any(token in action for token in ("jump", "leap", "hop", "bounce"))
                has_turn_cue_step = (
                    ("turn" in action)
                    or ("spin" in action)
                    or ("pivot" in action)
                    or has_turn_kind_primitive
                )
                if has_jump_cue:
                    jump_cue_step_total += 1
                    if beats is not None:
                        jump_cue_step_positive_beats += 1
                if has_turn_cue_step:
                    turn_cue_step_total += 1
                    if has_axis_this_step:
                        turn_cue_step_axis += 1

                if prev_seen:
                    transition_total += 1
                    prev_end_present = bool(prev_end_foot) and not is_placeholder_value(prev_end_foot)
                    if prev_end_present and start_foot_present:
                        transitions_with_foot_markers += 1
                        if normalize_value(prev_end_foot) != start_foot_token:
                            transition_foot_continuity_violations += 1
                    prev_start_present = bool(prev_start_foot) and not is_placeholder_value(prev_start_foot)
                    if prev_start_present and start_foot_present:
                        transitions_with_start_foot += 1
                        if normalize_value(prev_start_foot) == start_foot_token:
                            transition_start_foot_repeat_violations += 1

                prev_start_foot = start_foot
                prev_end_foot = end_foot
                prev_seen = True
                expected_count += 1
            if figure_has_steps:
                figure_beat_totals.append(figure_sum)
            else:
                figures_without_steps += 1
        ...
        out[file_path.resolve().as_posix()] = {
            ...
            "stepsMissingWho": steps_missing_who,
            "stepsMissingAction": steps_missing_action,
            "stepsMissingPrimitive": steps_missing_primitive,
            "stepsMissingStartFoot": steps_missing_start_foot,
            "stepsMissingEndFoot": steps_missing_end_foot,
            "stepFootPairsSeen": step_foot_pairs_seen,
            "stepsSameStartEndFoot": steps_same_start_end_foot,
            "stepCountSequenceViolations": step_count_sequence_violations,
            "figuresWithoutSteps": figures_without_steps,
            "transitionTotal": transition_total,
            "transitionsWithFootMarkers": transitions_with_foot_markers,
            "transitionFootContinuityViolations": transition_foot_continuity_violations,
            "transitionsWithStartFoot": transitions_with_start_foot,
            "transitionStartFootRepeatViolations": transition_start_foot_repeat_violations,
            "stepsNonPositiveBeats": steps_non_positive_beats,
            "stepBeatsSeen": step_beats_seen,
            "jumpCueStepTotal": jump_cue_step_total,
            "jumpCueStepPositiveBeats": jump_cue_step_positive_beats,
            "turnCueStepTotal": turn_cue_step_total,
            "turnCueStepAxis": turn_cue_step_axis,
...
def evaluate_rule_non_couple_requires_dancers_count(
    meta: dict[str, Any],
) -> tuple[bool, bool, list[str]]:
    formation = str(meta.get("formationKind") or "")
    applicable = formation in {"line", "twoLinesFacing", "circle"}
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasDancersCount")), ["missing_non_couple_dancers_count_contract"]


def evaluate_rule_non_couple_requires_primitive_dir(
    meta: dict[str, Any],
) -> tuple[bool, bool, list[str]]:
    formation = str(meta.get("formationKind") or "")
    applicable = formation in {"line", "twoLinesFacing", "circle"}
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasPrimitiveDir")), ["missing_non_couple_primitive_dir_contract"]


def evaluate_rule_non_couple_requires_primitive_frame(
    meta: dict[str, Any],
) -> tuple[bool, bool, list[str]]:
    formation = str(meta.get("formationKind") or "")
    applicable = formation in {"line", "twoLinesFacing", "circle"}
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasPrimitiveFrame")), ["missing_non_couple_primitive_frame_contract"]


def evaluate_rule_circle_requires_axis(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = str(meta.get("formationKind") or "") == "circle"
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasPrimitiveAxis")), ["missing_circle_axis_contract"]


def evaluate_rule_line_like_requires_axis(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = str(meta.get("formationKind") or "") in {"line", "twoLinesFacing"}
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasPrimitiveAxis")), ["missing_line_like_axis_contract"]


def evaluate_rule_couple_requires_axis(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = str(meta.get("formationKind") or "") == "couple"
    if not applicable:
        return False, True, []
    return True, bool(meta.get("hasPrimitiveAxis")), ["missing_couple_axis_contract"]


def evaluate_rule_step_who_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsMissingWho", 0)) == 0, ["missing_step_who"]


def evaluate_rule_step_action_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsMissingAction", 0)) == 0, ["missing_step_action"]


def evaluate_rule_step_geo_primitive_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsMissingPrimitive", 0)) == 0, ["missing_step_geo_primitive"]


def evaluate_rule_step_count_sequence_per_figure(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return (
        True,
        int(meta.get("stepCountSequenceViolations", 0)) == 0,
        ["invalid_step_count_sequence"],
    )


def evaluate_rule_figures_have_steps(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("figuresWithoutSteps", 0)) == 0, ["figure_without_steps"]


def evaluate_rule_step_start_foot_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsMissingStartFoot", 0)) == 0, ["missing_step_start_foot"]


def evaluate_rule_step_end_foot_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepTotal", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsMissingEndFoot", 0)) == 0, ["missing_step_end_foot"]


def evaluate_rule_step_support_change_required(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("stepFootPairsSeen", 0)) > 0
    if not applicable:
        return False, True, []
    return True, int(meta.get("stepsSameStartEndFoot", 0)) == 0, ["same_start_end_foot"]


def evaluate_rule_step_transition_foot_continuity(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("transitionsWithFootMarkers", 0)) > 0
    if not applicable:
        return False, True, []
    return (
        True,
        int(meta.get("transitionFootContinuityViolations", 0)) == 0,
        ["foot_transition_continuity_violation"],
    )


def evaluate_rule_step_start_foot_alternation(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = int(meta.get("transitionsWithStartFoot", 0)) > 0
    if not applicable:
        return False, True, []
    return (
        True,
        int(meta.get("transitionStartFootRepeatViolations", 0)) == 0,
        ["start_foot_repeat_violation"],
    )


def evaluate_rule_jump_cues_positive_beats(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    jump_total = int(meta.get("jumpCueStepTotal", 0))
    applicable = jump_total > 0
    if not applicable:
        return False, True, []
    return (
        True,
        int(meta.get("jumpCueStepPositiveBeats", 0)) == jump_total,
        ["jump_cue_missing_positive_beats"],
    )


def evaluate_rule_turn_cue_axis_coverage_min(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    turn_total = int(meta.get("turnCueStepTotal", 0))
    applicable = turn_total > 0
    if not applicable:
        return False, True, []
    covered = int(meta.get("turnCueStepAxis", 0))
    ratio = float(covered) / float(turn_total) if turn_total > 0 else 0.0
    # M17 introduces a minimum per-step axis realism signal for rotational cues.
    return True, ratio >= 0.15, ["low_turn_axis_step_coverage"]


def evaluate_rule_couple_realism_bundle(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    applicable = str(meta.get("formationKind") or "") == "couple"
    if not applicable:
        return False, True, []
    woman_side_ok = str(meta.get("womanSide") or "") in {"left", "right"}
    ok = (
        woman_side_ok
        and bool(meta.get("hasPairRelationship"))
        and bool(str(meta.get("holdKind") or ""))
        and bool(meta.get("hasPrimitiveAxis"))
    )
    return True, ok, ["couple_realism_bundle_incomplete"]


def evaluate_rule_non_couple_realism_bundle(meta: dict[str, Any]) -> tuple[bool, bool, list[str]]:
    formation = str(meta.get("formationKind") or "")
    applicable = formation in {"line", "twoLinesFacing", "circle"}
    if not applicable:
        return False, True, []
    ok = bool(meta.get("hasDancersCount")) and bool(meta.get("hasPrimitiveAxis"))
    if formation in {"line", "twoLinesFacing", "circle"}:
        ok = ok and bool(meta.get("hasPrimitiveDir")) and bool(meta.get("hasPrimitiveFrame"))
    if formation == "circle":
        ok = ok and bool(meta.get("hasPrimitivePreserveOrder"))
    return True, ok, ["non_couple_realism_bundle_incomplete"]
...
RULE_SPECS: list[dict[str, Any]] = [
    ...
    {
        "key": "rule:non_couple_requires_dancers_count",
        "name": "non_couple_requires_dancers_count",
        "description": "Line/circle/two-lines formations require dancers count in the geometry contract.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_non_couple_requires_dancers_count,
    },
    {
        "key": "rule:non_couple_requires_primitive_dir",
        "name": "non_couple_requires_primitive_dir",
        "description": "Line/circle/two-lines formations require directional primitive semantics.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_non_couple_requires_primitive_dir,
    },
    {
        "key": "rule:non_couple_requires_primitive_frame",
        "name": "non_couple_requires_primitive_frame",
        "description": "Line/circle/two-lines formations require primitive frame semantics.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_non_couple_requires_primitive_frame,
    },
    {
        "key": "rule:circle_requires_axis",
        "name": "circle_requires_axis",
        "description": "Circle formations require explicit axis semantics for rotational topology.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_circle_requires_axis,
    },
    {
        "key": "rule:line_like_requires_axis",
        "name": "line_like_requires_axis",
        "description": "Line and two-lines-facing formations require explicit axis semantics.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_line_like_requires_axis,
    },
    {
    ...
    {
        "key": "rule:couple_requires_axis",
        "name": "couple_requires_axis",
        "description": "Couple formations require axis semantics for partner-relative turn logic.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_couple_requires_axis,
    },
    {
        "key": "rule:couple_realism_bundle",
        "name": "couple_realism_bundle",
        "description": "Couple formations require coherent orientation, relationship, hold, and axis realism bundle.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_couple_realism_bundle,
    },
    {
        "key": "rule:non_couple_realism_bundle",
        "name": "non_couple_realism_bundle",
        "description": "Non-couple formations require coherent count/direction/frame/axis realism bundle.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_non_couple_realism_bundle,
    },
    {
    ...
    {
        "key": "rule:step_who_required",
        "name": "step_who_required",
        "description": "Each step must declare performer scope (`who`).",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_who_required,
    },
    {
        "key": "rule:step_action_required",
        "name": "step_action_required",
        "description": "Each step must include action semantics.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_action_required,
    },
    {
        "key": "rule:step_geo_primitive_required",
        "name": "step_geo_primitive_required",
        "description": "Each step must include at least one geometry primitive.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_geo_primitive_required,
    },
    {
        "key": "rule:step_count_sequence_per_figure",
        "name": "step_count_sequence_per_figure",
        "description": "Step counts within each figure must follow contiguous ascending sequence.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_count_sequence_per_figure,
    },
    {
        "key": "rule:figures_have_steps",
        "name": "figures_have_steps",
        "description": "Every figure must contain at least one step.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_figures_have_steps,
    },
    {
        "key": "rule:step_start_foot_required",
        "name": "step_start_foot_required",
        "description": "Each step must declare start-foot support marker.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_start_foot_required,
    },
    {
        "key": "rule:step_end_foot_required",
        "name": "step_end_foot_required",
        "description": "Each step must declare end-foot support marker.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_end_foot_required,
    },
    {
        "key": "rule:step_support_change_required",
        "name": "step_support_change_required",
        "description": "Each step should transition support foot (startFoot != endFoot).",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_support_change_required,
    },
    {
        "key": "rule:step_transition_foot_continuity",
        "name": "step_transition_foot_continuity",
        "description": "Consecutive steps should continue support from prior endFoot to next startFoot.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_transition_foot_continuity,
    },
    {
        "key": "rule:step_start_foot_alternation",
        "name": "step_start_foot_alternation",
        "description": "Consecutive step start-foot markers should alternate for basic transition coherence.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_step_start_foot_alternation,
    },
    {
        "key": "rule:jump_cues_positive_beats",
        "name": "jump_cues_positive_beats",
        "description": "Jump/leap/hop cues require positive step beats values.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_jump_cues_positive_beats,
    },
    {
        "key": "rule:turn_cue_axis_coverage_min",
        "name": "turn_cue_axis_coverage_min",
        "description": "Turn cues require minimum per-step axis coverage for biomechanical plausibility.",
        "candidateKeys": [],
        "evaluate": evaluate_rule_turn_cue_axis_coverage_min,
    },
    {
...
def main() -> int:
    ...
    print(
        f"M17 VALIDATOR EXPANSION {'PASS' if ok else 'FAIL'} "
