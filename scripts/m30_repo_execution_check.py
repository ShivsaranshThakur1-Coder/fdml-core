#!/usr/bin/env python3
"""Deterministic M30 repository-cleanup execution gate."""
...
REQUIRED_BASELINE_GAP_IDS = {"M30-BLG-001", "M30-BLG-003", "M30-BLG-005"}


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Validate M30 repository cleanup execution state.")
    ...
    ap.add_argument("--report-out", default="out/m30_repo_execution_report.json")
    ap.add_argument("--required-active-milestone", default="M30")
    ap.add_argument("--required-work-id", default="PRG-281")
    ap.add_argument("--required-next-work-id", default="PRG-282")
    ...
    ap.add_argument("--required-ci-target", default="m30-repo-execution-check")
    ap.add_argument("--max-queued-gap-after", type=int, default=2)
    ap.add_argument("--max-open-queue-after", type=int, default=1)
    ap.add_argument("--max-dirty-delta", type=int, default=4)
...
def main() -> int:
    ...
    row_281 = find_work_row(rows, args.required_work_id)
    require(row_281 is not None, f"missing work item: {args.required_work_id}")
    status_281 = str((row_281 or {}).get("status", "")).strip().lower()
    checks.append(
        {
            "id": "work_item_281_done",
            "ok": status_281 == "done",
            "detail": f"{args.required_work_id}.status={status_281!r}",
        }
    )
    require(checks[-1]["ok"], checks[-1]["detail"])

    row_282 = find_work_row(rows, args.required_next_work_id)
    require(row_282 is not None, f"missing work item: {args.required_next_work_id}")
    status_282 = str((row_282 or {}).get("status", "")).strip().lower()
    checks.append(
        {
            "id": "work_item_282_present",
            "ok": status_282 in {"planned", "in_progress", "done"},
            "detail": f"{args.required_next_work_id}.status={status_282!r}",
        }
    )
    require(checks[-1]["ok"], checks[-1]["detail"])
    ...
    baseline_open_queue = int(baseline_metrics.get("openM30QueueCount", -1))
    ...
    if baseline_pycache > 0 or (baseline_tracked_changes + baseline_untracked_changes) > 0:
        expected_gap_ids.add("M30-BLG-001")
    if baseline_queued_gaps > 0:
        expected_gap_ids.add("M30-BLG-003")
    if baseline_open_queue > 0:
        expected_gap_ids.add("M30-BLG-005")
    post_governance_baseline_shape = (
        status_282 == "done" and baseline_open_queue == 0 and baseline_queued_gaps <= 0
    )
    checks.append(
        {
            "id": "baseline_has_required_prg281_backlog",
            "ok": expected_gap_ids.issubset(baseline_gap_ids)
            or REQUIRED_BASELINE_GAP_IDS.issubset(baseline_gap_ids)
            or post_governance_baseline_shape,
            "detail": (
                f"baselineGapIds={sorted(baseline_gap_ids)} expectedGapIds={sorted(expected_gap_ids)} "
                f"postGovernanceShape={post_governance_baseline_shape}"
            ),
        }
    )
    require(checks[-1]["ok"], checks[-1]["detail"])
    ...
    checks.append(
        {
            "id": "step_map_contains_prg281_command",
    ...
    program_plan_check = doc_token_check(
        ...
        ["M30", "PRG-281", args.required_ci_target, "out/m30_repo_execution_report.json"],
    )
    usage_check = doc_token_check(
        Path(args.usage_doc),
        ["PRG-281", f"make {args.required_ci_target}", "out/m30_repo_execution_report.json"],
    )
    ...
    current_dirty = int(git_metrics["trackedChanges"]) + int(git_metrics["untrackedChanges"])
    baseline_dirty = baseline_tracked_changes + baseline_untracked_changes
    dirty_delta = current_dirty - baseline_dirty
    ...
    checks.append(
        {
            "id": "dirty_noise_delta_cap",
            "ok": dirty_delta <= args.max_dirty_delta,
            "detail": (
                f"baselineDirty={baseline_dirty} currentDirty={current_dirty} "
                f"delta={dirty_delta} maxDelta={args.max_dirty_delta}"
            ),
        }
    )
    require(checks[-1]["ok"], checks[-1]["detail"])
    ...
    for row in baseline_backlog:
        ...
        if linked == args.required_work_id and backlog_id == "M30-BLG-001":
            status = (
                "mitigated"
                if current_pycache <= baseline_pycache and dirty_delta <= args.max_dirty_delta
                else "open"
            )
            reason = (
                f"pycache baseline={baseline_pycache} current={current_pycache} "
                f"dirtyDelta={dirty_delta}"
            )
        elif linked == args.required_work_id and backlog_id == "M30-BLG-003":
        ...
        elif linked == args.required_work_id and backlog_id == "M30-BLG-005":
    ...
    report = {
        ...
        "metrics": {
            ...
            "baselineDirtyCount": baseline_dirty,
            "currentDirtyCount": current_dirty,
            "dirtyDelta": dirty_delta,
