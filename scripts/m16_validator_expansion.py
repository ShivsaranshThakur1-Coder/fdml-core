#!/usr/bin/env python3
"""Expanded full-corpus validator stack derived from M15 discovery candidates (M16 deepening)."""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Run expanded M16 validator stack from discovery candidates and publish failure taxonomy."
    )
    ap.add_argument(
        "--input-dir",
    ...
    ap.add_argument(
        ...
        help="path to M15 validator candidate report (M16 reuses this evidence ledger)",
    )
    ap.add_argument(
        "--report-out",
        default="out/m16_validator_expansion_report.json",
        help="output path for validator expansion report",
    )
    ap.add_argument(
        "--label",
        default="m16-validator-expansion",
        help="report label",
    )
    ap.add_argument(
        "--min-total-files",
...
def fail(msg: str) -> int:
    print(f"m16_validator_expansion.py: {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    print(
        f"M16 VALIDATOR EXPANSION {'PASS' if ok else 'FAIL'} "
